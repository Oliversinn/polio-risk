...
outbreaks_title_map <- function(LANG_TLS,COUNTRY_NAME,YEAR_LIST,admin1,var) {
  YEAR_1=YEAR_LIST[1];YEAR_2=YEAR_LIST[2];YEAR_3=YEAR_LIST[3];YEAR_4=YEAR_LIST[4];YEAR_5=YEAR_LIST[5];
  var_text <- case_when(
    var == "outbreaks_score" ~ paste0(lang_label_tls(LANG_TLS,"outbreaks_title_map_total_pr")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "polio" ~ paste0(lang_label_tls(LANG_TLS, "oubreaks_title_map_disease"), ' ' ,lang_label_tls(LANG_TLS,"outbreaks_polio")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "measles" ~ paste0(lang_label_tls(LANG_TLS, "oubreaks_title_map_disease"), ' ' ,lang_label_tls(LANG_TLS,"outbreaks_measles")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "rubella" ~ paste0(lang_label_tls(LANG_TLS, "oubreaks_title_map_disease"), ' ' ,lang_label_tls(LANG_TLS,"outbreaks_rubella")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "diphtheria" ~ paste0(lang_label_tls(LANG_TLS, "oubreaks_title_map_disease"), ' ' ,lang_label_tls(LANG_TLS,"outbreaks_diphtheria")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "yellow_fever" ~ paste0(lang_label_tls(LANG_TLS, "oubreaks_title_map_disease"), ' ' ,lang_label_tls(LANG_TLS,"outbreaks_yellow_fever")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "tetanus" ~ paste0(lang_label_tls(LANG_TLS, "oubreaks_title_map_disease"), ' ' ,lang_label_tls(LANG_TLS,"outbreaks_tetanus")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")")
  )
  return(var_text)
}
...
outbreaks_plot_map_data <- function(
  ...
  map_data$risk_level <- get_risk_level(LANG_TLS,CUT_OFFS,indicator,map_data$outbreaks_score,map_data$population_and_pfa_bool)
  
  if (var_to_summarise == "outbreaks_score") {
  ...
  } else if (var_to_summarise %in% c("polio", "measles", 
                                     "rubella", "diphtheria",
                                     "yellow_fever", "tetanus")) {
...
outbreaks_get_data_table <- function(LANG_TLS,YEAR_LIST,CUT_OFFS,data,admin1_id, pop_filter, risk_filter=toupper(lang_label("filter_all"))) {
  data$risk_level <- get_risk_level(LANG_TLS,CUT_OFFS,"outbreaks_score",data$outbreaks_score, data$population_and_pfa_bool)
  
  data <- data %>% 
    select(`ADMIN1 GEO_ID`,
           ADMIN1,ADMIN2,outbreaks_score,risk_level, POB1, POB5, POB15,
  ...
  if (admin1_id == 0) {
    ...
    colnames(data) <- c(lang_label_tls(LANG_TLS,"table_admin1_name"),
                        ...
                        lang_label_tls(LANG_TLS,"risk_level"),
                        "POB1", "POB5", "POB15",
                        lang_label_tls(LANG_TLS,"outbreaks_polio"),
                        lang_label_tls(LANG_TLS, "outbreaks_measles"),
                        lang_label_tls(LANG_TLS, "outbreaks_rubella"),
                        lang_label_tls(LANG_TLS,"outbreaks_diphtheria"),
                        lang_label_tls(LANG_TLS,"outbreaks_yellow_fever"),
                        lang_label_tls(LANG_TLS,"outbreaks_tetanus")
    )
  } else {
    ...
    datatable(
      ...
      options = list(
        ...
        buttons = list(
          ...
          list(extend='csv',filename=paste(lang_label_tls(LANG_TLS,"data"),lang_label_tls(LANG_TLS,"outbreaks_score"),admin1_id)),
          list(extend='excel', filename=paste(lang_label_tls(LANG_TLS,"data"),lang_label_tls(LANG_TLS,"outbreaks_score"),admin1_id))
        ),
        class = "display"
      )
    ) %>% formatStyle(
